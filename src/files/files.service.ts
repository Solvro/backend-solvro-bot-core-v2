...
export class FilesService {
  ...
  public async generateAttendanceFile(
    meetingId: number,
  ): Promise<string | null> {
    ...
    const userInfo = attendees.map((attendee) => {
      ...
      return {
        ...
        serverNickname: member
          ? member.displayName || 'No Nickname'
          : 'Not in Server',
      };
    });

    const header = 'discordId,globalName,serverNickname';
    const rows = userInfo.map(
      (info) => `${info.discordId},${info.globalName},${info.serverNickname}`,
    );
  ...
  public async generateMeetingSummaryFile(
    meetingId: number,
  ): Promise<string | null> {
  ...
  public async generateMeetingTranscriptFile(meetingId: number) {
    ...
    if (
      ...
      meeting.transcriptionChunks.length === 0
    ) {
      return null;
    }
    ...
    for (const chunk of meeting.transcriptionChunks) {
      if (!userNames[chunk.speakerDiscordId]) {
        ...
        userNames[chunk.speakerDiscordId] = member
          ? member.displayName
          : `User#${chunk.speakerDiscordId}`;
      }
    }
