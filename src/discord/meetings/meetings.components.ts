...
import { ActionRowBuilder, AttachmentBuilder, MessageFlags, ModalBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle } from "discord.js";
...
export class MeetingsComponents {
  ...
  public async onSummaryButton(@necord.Context() [interaction]: necord.ButtonContext) {
    ...
    if (meetings.length === 0) {
      await interaction.editReply({
        content: "❌ No past meetings with processed recordings found. Mind that after stopping a meeting, it may take a few moments for the recording to be processed."
      });
      return;
    }
  ...
  public async onSummarySelectMeeting(@necord.Context() [interaction]: necord.StringSelectContext) {
    ...
    if (!summary) {
      await interaction.editReply({
        content: "❌ No summary found for the selected meeting. Mind that it may take a few moments after stopping a meeting for the summary to be generated."
      });
      return;
    }
    ...
    if (summary.length <= 2000) {
      ...
      const chunks = this.meetingsService.chunkStringRespectingLinesAndWords(summary)
  ...
  public async onTranscriptionSelectMeeting(@necord.Context() [interaction]: necord.StringSelectContext) {
    ...
    if (!transcription) {
      await interaction.editReply({
        content: "❌ No transcription found for the selected meeting. Mind that it may take a few moments after stopping a meeting for the transcription to be generated."
      });
      return;
    }
