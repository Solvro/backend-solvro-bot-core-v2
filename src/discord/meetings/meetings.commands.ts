...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Client, TextChannel, EmbedBuilder } from "discord.js";
...
export class MeetingsCommands {
    constructor(private meetingsService: MeetingsService, private readonly client: Client) { }
    ...
    public async onWeeklyControlPanel(@necord.Context() [interaction]: necord.SlashCommandContext) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#0099ff')
            .addFields(
                { name: '🟢 Start Session', value: 'Starts a new weekly session, including voice recording and attendance tracking.', inline: true },
                { name: '🔴 Stop Session', value: 'Stops the current session and processes the recording.', inline: true },
                { name: '\u200B', value: '\u200B' }, // Spacer
                { name: 'Additional Controls', value: 'Use the buttons below to access transcription, attendance, and summary features.' }
            )
